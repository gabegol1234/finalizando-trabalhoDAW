...
class clientesDAO extends Connection
{
    ...
    public function editar(Clientes $clientes)
    {
        ...
        $query = mysqli_query($this->conexao, "SELECT * FROM clientes WHERE cpf = '" . $clientes->getCpf() . "' AND idCliente != '" . $clientes->getIdCliente() . "'");
        if (mysqli_fetch_assoc($query)) {
            return 2;
        }
        $query = mysqli_query($this->conexao, "SELECT * FROM clientes WHERE usuario = '" . $clientes->getUsuario() . "' AND idCliente != '" . $clientes->getIdCliente() . "'");
        if (mysqli_fetch_assoc($query)) {
            return 3;
        }
