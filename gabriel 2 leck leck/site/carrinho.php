...
<table border>
    ...
    <tbody>
        ...
        foreach ($_SESSION["carrinho"] as $linha) {
            // Chama a função retornarUm() para obter informações do produto
            $retorno = $objProdutosDAO->retornarUm($linha);
            
            // Verifica se a função retornou um array válido com os índices necessários
            if ($retorno && is_array($retorno) && isset($retorno["nome"], $retorno["preco"])) {
                ?>
                <tr>
                    <td><?= htmlspecialchars($retorno["nome"]); ?></td>
                    <td>R$ <?= number_format($retorno["preco"], 2, ',', '.'); ?></td>
                    <td><input type="number" name="quantidade<?=$linha;?>"  min="1"></td>
                </tr>
                <?php
            } else {
